...
void Mesh::CreateBuffers(Vertex* vertices, int numVertices, UINT* indices, int numIndices)
{
	...
	indexBuffer = Graphics::CreateStaticBuffer(sizeof(UINT), numIndices, indices);

	//set up views
	vbView.StrideInBytes = (UINT)sizeof(Vertex);
	vbView.SizeInBytes = (UINT)(sizeof(Vertex) * numVertices);
...
Mesh::Mesh(Vertex* verts, int numVertices, UINT* indices, int numIndices)
{
